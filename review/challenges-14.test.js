...
const createServer = () => {
  ...
  app.delete('/things/1', (request, response) => response.status(405).send('Hello'));
  app.get('*', (request, response) => response.status(404).send('fail'));
...
let biggerThanLuke = (arr) => {
  let bigPeopleArray = arr
    .filter(person => parseInt(person.mass) > parseInt(arr[0].mass))
    .reduce((acc, val) => {
      acc.push(val.name);
      return acc;
    }, []);
  return bigPeopleArray.join(' - ');
};
